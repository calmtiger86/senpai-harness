...
- Build Gate 8개 조건이 다 채워지기 전에 "이제 진행합니다"라고 사용자에게 말하지 않습니다.
...
아래 "Build Gate" 절의 8개 조건을 하나씩 짚어 사용자에게 보여줍니다. 8개가 전부 채워졌을 때만 다음 단계로 넘어갑니다. 하나라도 비었으면 어느 조건이 왜 안 됐는지 말하고, 해당 회의(Discovery/Scope/Build Readiness)로 돌아갑니다.
...
사용자가 최종 승인하면, Builder(또는 다음 실행 단계)에게 다음을 넘깁니다.

- Phase Plan 경로 (`vault/10_Projects/{project}/Phase Plan.md`)
- "작업 체크리스트" 항목만 = 승인된 작업 범위. 이 목록 밖의 파일/기능은 Builder가 건드릴 수 없습니다.
- "완료 증거" 목록 = Evidence Reviewer가 나중에 확인할 기준
- 새 의존성 설치, 인증/결제/배포/데이터 삭제는 계획에 명시적으로 없으면 여전히 금지라는 점

이 스킬의 역할은 여기서 끝납니다. 실제 코드 작성부터는 Builder(Project Explorer → Builder → Code Diagnostics → Evidence Reviewer → Memory Librarian, docs/02_PRODUCT_SPEC.md "4. Guided Work 흐름")가 이어받습니다.
...
이 8개는 docs/06_HOOKS_SPEC.md의 PreToolUse 차단 조건("사용자 승인 전 제품 코드 수정 금지", "계획 밖 파일 수정 금지")이 실제로 무엇을 막으려는 것인지를 구체화한 목록입니다. Guided Plan 스킬은 이 8개를 대화 속에서 확인하고 보여주는 역할이고, 실제로 코드 쓰기를 막거나 허용하는 것은 다음 절의 스크립트입니다.

## 실제 집행자는 누구인가 — approval-gate.js

**중요: 이 스킬이 "8/8 통과했습니다, 진행하세요"라고 말하는 것과, 실제로 Builder의 Write/Edit/Bash가 허용되는 것은 별개입니다.**
...
- **Build Gate 8개 중 일부 미충족**: 어느 조건이 왜 비었는지 말하고, 그 조건을 채울 수 있는 이전 회의로 돌아갑니다. 억지로 다음 단계로 넘기지 않습니다.
