...
- ["자율주행"을 자제 시키는 하네스](#자율주행을-자제-시키는-하네스)
- [무엇을 해주나](#무엇을-해주나)
- [설치](#설치)
- [라이선스](#라이선스)
...
그때 우리가 선배들에게 자주 들었던 말이 있죠.

"모르면 물어보면서 해. 그런데 이런것 까지 하나하나 알려줘야 해?"

센빠이 하네스는 모를 것 같은걸 먼저 물어봐주고, 결정된 내용을 친절하게 알려준 뒤 작업에 들어 갑니다.

현실에는 없던 그런 이상적인 선배의 역할을 센빠이 하네스는 해줍니다.

바이브코딩을 처음 접하는 비개발자에게 가장 친절한 센빠이 하네스.

Senpai Harness는 먼저 묻고, 쉽게 설명하고, 함께 결정하고, '승인된 범위만' 만들고,

모든 결정과 실패를 Obsidian(마크다운 메모 앱)에 남기는 AI 선배 하네스입니다.

## 누구를 위한 것인가

이런 사람들을 위해 만들었습니다.

코딩 문법을 하나도 모르는 사람, 터미널이나 Claude Code를 오늘 처음 켜보는 사람, 바이브코딩(대화하듯 AI에게 코드를 맡기는 방식)으로 앱이나 챗봇, 자동화를 만들어보고 싶은 사람, 그리고 AI가 만들어주는 결과를 따라가기 벅차서 바이브코딩을 포기한 적 있는 사람.

이미 코딩에 익숙하다면, 이 하네스가 요구하는 "먼저 묻고 승인받는" 절차가 오히려 느리게 느껴질 수 있습니다.

Senpai Harness는 속도보다 '끝까지 따라갈 수 있는 것'을 목표로 만들었습니다.

## "자율주행"을 자제 시키는 하네스

바이브코딩은 AI가 알아서 코드를 짜주는게 편리하긴 하지만, 실제로는 추론을 통해 그럴듯해 보이는 방향으로 스스로 의사결정을 하고 진행하는 경우가 많습니다.

그대로 '자율주행'을 AI가 혼자 판단해서 코딩하도록 두면 우리가 손댈 수 없는 꼬여 버린 스파게티 코딩을 만들어 냅니다.

스스로 추론하고 '자율주행'하는 대신 AI가 자동으로 올바른 회의를 열고, 사용자가 모르는 결정을 이해할 수 있는 선택지로 보여주고, '사용자가 승인한 범위 안에서만' 코드를 쓰도록 하는 하네스입니다.
